...
class PauseSubState extends MusicBeatSubstate
{
	...
	{
		super();
		// Show the mouse cursor while paused, but remember previous state to restore later
	...
	override function update(elapsed:Float)
	{
		...
		{
			if (item.isMenuItem && FlxG.mouse.overlaps(item))
			{
				if(curSelected != item.targetY)
				{
					FlxG.sound.play(Paths.sound('scrollMenu'), 0.4);
					curSelected = item.targetY;
					changeSelection();
				}
	...
	override function destroy()
	{
		...
		// restore mouse visibility to what it was before pausing
		FlxG.mouse.visible = prevMouseVisible;

		super.destroy();
	}

	function selectAndPress()
	{
